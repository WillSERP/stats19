...
For installation and the code, see its home in rOpenSci: https://github.com/ropensci/stats19.
...
To finish with a map, we can plot the `crashes_wy` on a LeafletJS map using Rstudio's `leaflet` package:
...
leaflet(data = wy_lonlat$geometry, height = "280px") %>%
  ...
  addCircleMarkers(radius = 0.5, color = "blue", weight = 1) 
```
